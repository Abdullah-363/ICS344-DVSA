// VULNERABLE VERSION - Before Fix (Lessons #1 and #9)
// Vulnerability: Uses node-serialize library which allows unsafe deserialization
// of attacker-controlled input, enabling Remote Code Execution (RCE)

const serialize = require('node-serialize'); // VULNERABLE: allows RCE via _$$ND_FUNC$$_ marker
...
exports.handler = (event, context, callback) => {
    var req = serialize.unserialize(event.body);    // VULNERABLE: executes attacker-controlled JS functions
    var headers = serialize.unserialize(event.headers); // VULNERABLE: same issue
